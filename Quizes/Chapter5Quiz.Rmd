...
#C
print("P(150 ≤ X ≤ 115)")
pbinom(150,170,.60)-pbinom(115,170,.60)
```
...
Consider the following discrete probability distribution.
    ...
    x           15     22    34    40
    P(X=x)    0.15   0.54  0.21  0.10
    

a. Is this a valid probability distribution?

```{r}
sum(c(0.15,0.54,0.21,0.1))
print("Yes it adds to 1")
```
...
c. What is the probability that the random variable X is between 9 and 30? (Round your answer to 2 decimal places.)

```{r}
0.15+0.54
```

d. What is the probability that the random variable X is greater than 18? (Round your answer to 2 decimal places.)
...
A basketball player is fouled while attempting to make a basket and receives two free throws. The opposing coach believes there is a 57% chance that the player will miss both shots, a 22% chance that he will make one of the shots, and a 21% chance that he will make both shots.
